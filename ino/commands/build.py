...
#DO NOT PLACE YOUR SKETCHES OR ENERGIA DIRECTORY WITHIN CYGWIN FILESYSTEM
#/home, /bin, etc. NATIVE GCC WILL NOT UNDERSTAND THOSE PATHS
def to_windows_path(unix_path):
    windows_path = subprocess.check_output(["cygpath", "-w", unix_path])
    windows_path = re.sub(r'(\r\n|\n|\r)\s*(\r\n|\n|\r)*',r'',windows_path)
    windows_path = re.sub(r'\\' ,r'/', windows_path)
    return windows_path
...
class Build(Command):
    ...
    *.[c|cpp|pde|ino] files. They are compiled and linked into resulting
    ...
    if "CYGWIN" in platform.platform():
        default_cppflags = '-Ic:/ti/gcc/include -Ic:/Users/Mario/Desktop/energia-0101E0015/hardware/msp430/variants/launchpad -Os -Wall -ffunction-sections -fdata-sections -g -DMSP430'
        default_cflags = ''
        default_cxxflags = '-fno-exceptions -fno-rtti'
        default_ldflags = '-Lc:/ti/gcc/include/ -Os --gc-sections -u,main'
    else:
        default_cppflags = '-I/home/mario/msp430/include -I/home/mario/energia-0101E0015/hardware/msp430/variants/launchpad -Os -Wall -ffunction-sections -fdata-sections -g -DMSP430'
        default_cflags = ''
        default_cxxflags = '-fno-exceptions -fno-rtti'
    ...
    def recursive_inc_lib_flags(self, libdirs):
        ...
        for d in libdirs:
            if "CYGWIN" in self.e['runenv']:
                flags.append('-I' + to_windows_path(d))
                flags.extend('-I' + to_windows_path(subd) for subd in list_subdirs(d, recursive=True, exclude=['examples']))
            else:
                flags.append('-I' + d)
    ...
    def _scan_dependencies(self, dir, lib_dirs, inc_flags):
        ...
        # libraries dirname
        if "CYGWIN" in self.e['runenv']:
            regexes = dict((lib, re.compile(r'\s' + to_windows_path(lib) + os.path.sep)) for lib in lib_dirs)
        else:
            ...
            for line in f:
                for lib, regex in regexes.iteritems():
                    if regex.search(line) and lib != dir:
                        if "CYGWIN" in self.e['runenv']:
                            used_libs.add(to_windows_path(lib))
                        else:
                            used_libs.add(lib)
